...
function Repair-StateDbThreadCwds {
    ...
    if (!(Test-Path $StateDbPath)) {
        Write-Warning "Codex state DB not found: $StateDbPath"
        return
    }

    $python = Get-Command python -ErrorAction SilentlyContinue
    if (!$python) {
        Write-Warning "Python was not found, so state DB cwd repair was skipped."
        return
    }
...
function Repair-SessionMetadata {
    ...
    if (!(Test-Path $SessionsDir)) {
        Write-Warning "Codex sessions directory not found: $SessionsDir"
        return
    }

    $python = Get-Command python -ErrorAction SilentlyContinue
    if (!$python) {
        Write-Warning "Python was not found, so session metadata repair was skipped."
        return
    }
...
function Repair-DesktopHistoryHints {
    if (!(Test-Path $GlobalStatePath)) {
        Write-Warning "Codex Desktop global state not found: $GlobalStatePath"
        return
    }
    if (!(Test-Path $SessionsDir)) {
        Write-Warning "Codex sessions directory not found: $SessionsDir"
    ...
    Write-Host "Repaired Codex Desktop history hints: sessions=$found, added=$added, updated=$updated, parse_errors=$errors"
    Repair-SessionMetadata -DesiredProvider $desiredProvider
    Repair-StateDbThreadCwds -DesiredProvider $desiredProvider
    Write-Host "Restart Codex Desktop if the sidebar still shows stale project chat lists."
}
...
function Switch-ToApimaster {
    ...
    if (!$ApiKey) {
        $secure = Read-Host "Paste APIMaster API key" -AsSecureString
    ...
    if (!$ApiKey) {
        throw "APIMaster API key is required."
    }
    ...
    Write-Host "Switched Codex to APIMaster: model=$Model, base_url=$BaseUrl"
    Repair-DesktopHistoryHints
    Write-Host "Conversation history is untouched. Restart Codex Desktop or open a new turn if the app has cached provider settings."
}
...
function Switch-ToOfficial {
    ...
    if (Test-Path $OfficialAuthPath) {
        ...
        Write-Warning "No saved official auth profile found. Use Codex login if the official subscription is not active."
    }

    Write-Host "Switched Codex to official subscription profile."
    Repair-DesktopHistoryHints
    Write-Host "Conversation history is untouched. Restart Codex Desktop or open a new turn if the app has cached provider settings."
}
...
function Show-Status {
    ...
    Write-Host "Codex provider: $providerLabel"
    Write-Host "Model: $modelName"
    Write-Host "Auth mode: $authMode"
    Write-Host "Switcher backups: $StateDir"
}
...
function Test-Apimaster {
    ...
    if (!$ApiKey) {
        throw "No APIMaster API key found. Run: .\switch-codex-provider.ps1 apimaster -ApiKey YOUR_KEY"
    }
    ...
    Write-Host "APIMaster /models OK. First models:"
...
switch ($Mode) {
    ...
    "save-official" { Save-OfficialProfile; Write-Host "Saved current Codex config/auth as official profile." }
    "test" { Test-Apimaster }
